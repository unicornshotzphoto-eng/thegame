...
def build_category_summary(include_counts=True):
    categories = []
    for index, (code, label) in enumerate(Question.CATEGORY_CHOICES, start=1):
        data = {
            'id': index,
            'category': code,
            'name': label,
        ...
        if include_counts:
            data['question_count'] = Question.objects.filter(category=code).count()
...
class GroupMessagesView(APIView):
    ...
    def get(self, request, group_id):
        ...
        messages = group.messages.all()[:100]
...
class CalendarInviteView(APIView):
    ...
    def post(self, request, calendar_id):
        ...
        if friends.count() != len(friend_ids):
            return Response({'error': 'Some friends not found'}, status=404)

        calendar.members.add(*friends)
        return Response({'message': f'{friends.count()} friends invited'}, status=200)
...
class QuestionsListView(APIView):
    ...
    def get(self, request, category=None):
        category_param = category or request.query_params.get('category')

        if category_param:
            questions = Question.objects.filter(category=category_param)
            serializer = QuestionSerializer(questions, many=True)
            if category:
                return Response(serializer.data, status=200)
            return Response({'questions': serializer.data}, status=200)

        categories = build_category_summary(include_counts=True)
...
class CreateGameSessionView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            if not participants:
                return Response({'error': 'No participants found'}, status=404)
    ...
    def post(self, request):
        ...
        try:
            ...
            for participant in participants:
                GameTurn.objects.get_or_create(
                    round=round_obj,
                    player=participant
                )
